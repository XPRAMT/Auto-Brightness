...
from monitor_ddc import (
    MONITORINFOEXW,
    _rect_to_tuple,
    run_ddc_with_timeout,
    get_windows_active_display_entries,
    get_windows_active_monitor_names,
    append_missing_windows_display_placeholders,
...
class _MonitorHotplugWatcher(QtCore.QObject):
    ...
    def _wmi_event_loop(self):
        try:
            ...
            watch_creation = wmi_conn.watch_for(notification_type="Creation", wmi_class="Win32_DesktopMonitor", delay_secs=1)
            watch_deletion = wmi_conn.watch_for(notification_type="Deletion", wmi_class="Win32_DesktopMonitor", delay_secs=1)
...
class MonitorWidget(QtWidgets.QGroupBox):
    ...
    def __init__(self, monitor_wrapper, threadpool):
        ...
        self.auto_info_label = None
        if not isinstance(self.monitor, RemoteMonitorWrapper):
            self.auto_info_label = QtWidgets.QLabel(
                "畫面亮度: -- | 背光亮度: -- | 加權亮度: -- | 目標亮度: -- | 權重: -- | 來源: --"
            )
    ...
    def set_auto_info(self, text):
        if self.auto_info_label is not None:
...
class MainWindow(QtWidgets.QWidget):
    ...
    def _format_detection_line(self, label, names):
        names = [str(name) for name in names if is_valid_monitor_name(str(name))]
        if names:
    ...
    def _format_dxgi_targets(self):
        ...
        # 偵測紀錄不可同步呼叫 grab()；顯示器切換時這會阻塞 UI 並與 analyzer 競爭 camera。
    ...
    def _restore_known_monitors_from_settings(self):
        ...
        try:
            ...
            for i, name in enumerate(known_names):
                if not is_valid_monitor_name(name):
    ...
    def _do_hotplug_refresh(self):
        ...
        if self._is_quitting:
            return
        try:
            ddc_count = run_ddc_with_timeout(
    ...
    def schedule_dxgi_refresh(self):
        """將 DXGI 重建排回 UI event loop，合併同一波拓撲變更的多個請求。"""
        if self._is_quitting or getattr(self, "_refresh_in_progress", False):
            return
        if getattr(self, "_dxgi_refresh_scheduled", False):
            return
        self._dxgi_refresh_scheduled = True
        QtCore.QTimer.singleShot(500, self._run_scheduled_dxgi_refresh)

    def _run_scheduled_dxgi_refresh(self):
        self._dxgi_refresh_scheduled = False
    ...
    def save_settings(self):
        ...
        local_wrappers = [w for w in self.monitor_wrappers if not isinstance(w, RemoteMonitorWrapper)]
        valid_names = sorted(
            w.name for w in local_wrappers if is_valid_monitor_name(w.name)
        )
        data = {
